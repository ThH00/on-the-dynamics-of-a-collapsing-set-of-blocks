...
def parse_args():
    ...
    parser.add_argument('eN', type=float, help='normal restitution coefficient (float)')
    parser.add_argument('ntime', type=int, help='number of time steps (integer)')
    parser.add_argument('vX0', type=float, help='horizontal velocity of first block (float)')
    ...
    return args.n, args.eN, args.ntime, args.vX0, args.mu_val, args.output_path
...
# eN    input, normal coefficient of restitution
...
def get_gN(q,u,a):
    ...
    if bottom_block_fixed[leaves_counter] == 0:
        w[0] = 0.2/l_nd
...
def increment_saved_arrays():
    global q_save, u_save, X_save, gNdot_save, gammaF_save, AV_save, corners_save, bottom_block_fixed
    ...
    bottom_block_fixed = np.append(bottom_block_fixed, bottom_block_fixed[-1])
...
def solve(iter_start):
    ...
    while iter<ntime:
        ...
        try:
            ...
            if bottom_block_fixed[leaves_counter] == 0 and corners_save[leaves_counter,0,iter] == 4:
                bottom_block_fixed[leaves_counter] = iter
